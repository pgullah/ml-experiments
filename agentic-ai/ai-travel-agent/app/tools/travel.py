...
def build_travel_tools(
    *,
    ...
    currency_converter: CurrencyTool,
    calculator: BudgetingTool,
) -> list[BaseTool]:
    ...
    def search(query: str, success_prefix: str, failure_message: str) -> str:
        try:
            results = search_service.run(query)
    ...
    def search_attraction(city: str) -> str:
        ...
        results = search_service.run(f"Top tourist attractions in {city}")
        if results:
    ...
    def convert_currency(
        ...
        return currency_converter.convert_currency(
            amount,
            from_currency,
            to_currency,
        )
